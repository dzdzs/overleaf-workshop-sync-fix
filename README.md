# Overleaf Workshop: proxy and local replica fixes

This repository is a maintained patch set on top of Overleaf Workshop
`v0.15.10`. It fixes remote development environments where Overleaf is only
reachable through an HTTP proxy and where the LaTeX project is nested inside a
larger VS Code or Cursor workspace.

The patch also uploads changes made outside the editor, including terminal,
Git, and coding agent writes. See [PATCHES.md](PATCHES.md) for the exact changes,
verification evidence, and build instructions.
