...
export class SCMCollectionProvider extends vscode.Disposable {
    ...
    constructor(
        ...
        super(() => {
            this.scms.forEach(item => item.triggers.forEach(t => t.dispose()));
        });
    ...
    private async initSCMs() {
        ...
        for (const scmPersist of Object.values(scmPersists)) {
            ...
            if (scmProto!==undefined) {
                ...
                const baseUri = scmPersist.baseUri.startsWith('/') ? vscode.Uri.file(scmPersist.baseUri) : vscode.Uri.parse(scmPersist.baseUri);
        ...
        const alreadyRegistered = localBaseUri && this.scms.some(item => item.scm.baseUri.fsPath===localBaseUri.fsPath);
        if (localBaseUri?.scheme==='file' && matchesThisProject && !alreadyRegistered) {
            console.log(`Overleaf Workshop: associating local replica "${localBaseUri.path}"`);
            await this.createSCM(LocalReplicaSCMProvider, localBaseUri, true, true);
        }
    }
